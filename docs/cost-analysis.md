...
| 구분 | 비용 |
|---|---|
| 24시간 상시 운영 시 (월) | 약 $253 (약 ₩349,000, 환율 1,380원 가정) |
...
| WAF (WebACL + 룰 4개: Common/KnownBadInputs/SQLi/RateLimit) | $5/WebACL + $1/룰 | $9.00 |
| Secrets Manager (시크릿 1개) | $0.40/시크릿/월 | $0.40 |
...
**WAF는 프론트(CloudFront)가 아니라 백엔드 ALB에 붙임** — 정적 파일만 서빙하는 프론트보다, 로그인·검색·장바구니처럼 사용자 입력이 DB까지 가는 백엔드 API 쪽이 실제 공격 표면이라 판단. AWS 관리형 룰셋(Common/KnownBadInputs/SQLi) + IP당 5분 2,000건 요청 제한 룰을 붙였고, SQL 인젝션 패턴(`?search=1' OR '1'='1`)으로 실제 403 차단 확인.
...
**Secrets Manager는 값이 아니라 "구조"가 바뀐 것** — DB 비밀번호/JWT시크릿/Anthropic 키가 더 이상 로컬 디스크에 평문 YAML로 존재하지 않는다. Secrets Store CSI Driver + IRSA로 파드가 뜰 때 IAM 권한으로 직접 가져오고, `k8s/api-secret.yaml`(평문 파일)은 완전히 삭제했다. 비용은 시크릿 1개당 월 $0.40으로 미미하지만, 보안 구조 개선이 핵심.
